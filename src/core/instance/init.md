...
该文件暴露了一个 `initMixin` 方法，将 `_init` 函数混入Vue原型中，暴露了另一个 `resolveConstructorOptions` 方法，解析Constructor的配置项 options
...
### 2. `super` 关键字
> super这个关键字有两种用法：既可以当作函数使用，也可以当作对象使用。在这两种情况下，它的用法完全不同。

#### 当做函数

第一种情况，super作为函数调用时，代表父类的构造函数。ES6 要求，子类的构造函数必须执行一次super函数，否则会报错。
...
class B extends A {
  constructor() {
    super();
    console.log(super.title()); // AAA super指向A.prototype
  }
}
...
class B extends A {
  static getSuperTitle () {
    console.log(super.title()) // AAA super指向A
  }
}

B.getSuperTitle(); 
```
> 注意：上面的A中，如果title方法不是静态方法，则调用B.getSuperTitle()会报错，因为super指向的是构造函数A。
